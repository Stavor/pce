...
int nIters;
enum AlgType {tBFS = 0, tSSSP = 1};
enum AlgType algType = tBFS;
...
void usage(int argc, char **argv)
{
    ...
    printf("    -sssp -- sssp algorithm. Default algorithm is bfs \n");
    printf("    -bfs  -- bfs algorithm. Default algorithm is bfs \n");
...
int main(int argc, char **argv) 
{
    ...
    else if (algType == tBFS) {
        init_bfs(&g);
        marked = (uint8_t *)malloc(g.n * sizeof(uint8_t));
    }
    ...
    for (uint32_t i = 0; i < nIters; ++i) {
        ...
        if (algType == tSSSP) sssp(g.roots[i], &g, dist, &nedges);
        else if (algType == tBFS) bfs(marked, validateNLevels, &validateNLevelsLength, g.roots[i], &g, &nedges);
        ...
        if (rootNumberToValidate == i) {
            ...
            if (algType == tSSSP) writeDistance(outFilename, dist, g.n);
            else if (algType == tBFS) writeLevels(outFilename, validateNLevels, validateNLevelsLength);
        } 
    }
    ...
    if (algType == tSSSP) printf("%s: nIters = %d SSSP performance min = %.4f avg = %.4f max = %.4f MTEPS\n", inFilename, nNonZeroRoots, min_perf, avg_perf, max_perf);
    else if (algType == tBFS) printf("%s: nIters = %d BFS performance min = %.4f avg = %.4f max = %.4f MTEPS\n", inFilename, nNonZeroRoots, min_perf, avg_perf, max_perf);

    if (algType == tSSSP) finalize_sssp(&g);
    else if (algType == tBFS) finalize_bfs();
    freeGraph(&g);
